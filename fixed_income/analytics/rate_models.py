...
def weighted_fit(design: np.ndarray, y: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, float | None]:
...
def empty_model(reason: str, observations_used: int | None = None, lookback_days_used: int | None = None) -> dict[str, Any]:
    return {
...
def regress_duration(frame: pd.DataFrame, lookback_days: int, factor_columns: list[str]) -> dict[str, Any]:
    ...
    rate_beta_sum = sum(float(beta_map.get(series_id, 0.0)) for series_id in RATE_SERIES)
    return {
...
def regress_benchmark_duration(frame: pd.DataFrame, lookback_days: int, benchmark_duration: float) -> dict[str, Any]:
    minimum = max(MIN_OBSERVATIONS, lookback_days // 3)
    if len(frame) < minimum:
