            ...
            if compare_hist.empty:
                ...
                if merged.empty:
                    ...
                    rv_merged = merged.loc[
                        (merged.index.date >= rv_start_date) & (merged.index.date <= rv_end_date)
                    ].copy()
                    ...
                    if rv_merged.empty:
                        ...
                        if ratio_std != 0:
                            rv_merged["zscore"] = (rv_merged["ratio"] - ratio_mean) / ratio_std
                        else:
                            rv_merged["zscore"] = 0.0
                        ...
                        if len(ratio_returns.dropna()) > 1:
                            realized_vol = float(ratio_returns.std(ddof=0)) * (252 ** 0.5)
                        else:
                            realized_vol = 0.0
                        ...
                        if 0 < lag1_autocorr < 1:
                            half_life = -math.log(2) / math.log(lag1_autocorr)
                        else:
                            half_life = 0.0
                        ...
                        def compute_window_z(series, window):
                        ...
                        z_30d = compute_window_z(full_ratio, 30)
                        z_90d = compute_window_z(full_ratio, 90)
                        z_180d = compute_window_z(full_ratio, 180)
                        ...
                        if spread_std != 0:
                            rv_merged["beta_adj_z"] = (rv_merged["beta_adj_spread"] - spread_mean) / spread_std
                        else:
                            rv_merged["beta_adj_z"] = 0.0
                        ...
                        if full_ratio_std != 0:
                            full_z_series = (full_ratio - full_ratio_mean) / full_ratio_std
                        else:
                            full_z_series = pd.Series(0.0, index=full_ratio.index)
                        ...
                        def compute_forward_reversion_stats(horizon: int):
                            favorable_moves = []
                            for dt in event_dates:
                                ...
                                if future_idx >= len(full_ratio):
                                    continue
                                start_ratio = float(full_ratio.iloc[idx])
                                end_ratio = float(full_ratio.iloc[future_idx])
                                z0 = float(full_z_series.iloc[idx])
                                raw_move = (end_ratio / start_ratio - 1.0) * 100 if start_ratio != 0 else 0.0
                                favorable_move = (-1 if z0 > 0 else 1) * raw_move
                                favorable_moves.append(favorable_move)
                            if not favorable_moves:
                                return 0.0, 0.0, 0
                            avg_move = float(sum(favorable_moves) / len(favorable_moves))
                            hit_rate = float(sum(1 for x in favorable_moves if x > 0) / len(favorable_moves))
                            return avg_move, hit_rate, len(favorable_moves)

                        fwd_5_avg, fwd_5_hit, fwd_5_n = compute_forward_reversion_stats(5)
                        fwd_10_avg, fwd_10_hit, fwd_10_n = compute_forward_reversion_stats(10)
                        fwd_20_avg, fwd_20_hit, fwd_20_n = compute_forward_reversion_stats(20)
                        ...
                        for idx, (label, value) in enumerate(top_metrics):
                            with top_cols[idx % 3]:
                        ...
                        for idx, (label, value) in enumerate(secondary_metrics):
                            with secondary_cols[idx % 3]:
                                st.metric(label, value)
                        ...
                        for idx, (label, value) in enumerate(horizon_metrics):
                            with horizon_cols[idx % 3]:
                                st.metric(label, value)
                        ...
                        st.caption(
                            "RV modules active: ratio z-score, return spread, rolling correlation, rolling beta, regime labels, half-life proxy, vol-adjusted score, window comparison, liquidity overlay, and signal history."
                        )
                        ...
                        signal_history = signal_history.rename(columns={
                            "date": "DATE",
                            "zscore": "Z-SCORE",
                            "regime": "REGIME",
                            "cross": "CROSS",
                        })
                        signal_history = signal_history.tail(12)
                        ...
                        with st.expander("Show RV pair screener"):
                            ...
                            for candidate in rv_candidates:
                                ...
                                candidate_merged = candidate_merged.loc[
                                    (candidate_merged.index.date >= rv_start_date) & (candidate_merged.index.date <= rv_end_date)
                                ].copy()
                                ...
                                cand_corr = float(cand_ret_base.rolling(20).corr(cand_ret_comp).dropna().iloc[-1]) if not cand_ret_base.rolling(20).corr(cand_ret_comp).dropna().empty else 0.0
                                ...
                                cand_half_life_component = max(min(1 - abs(((-math.log(2) / math.log(cand_ratio_autocorr)) if 0 < cand_ratio_autocorr < 1 else 0.0) - 10) / 20, 1.0), 0.0) if 0 < cand_ratio_autocorr < 1 else 0.0
                                cand_stability = 100 * (
