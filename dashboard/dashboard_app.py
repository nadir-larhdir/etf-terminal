...
class DashboardApp:
    ...
    def run(self) -> None:
        """Apply theme, load etf_universe, handle navigation, and delegate to the active page."""
        apply_dashboard_theme()

        cache_key = app_cache_key(self.etf_universe_store.engine)
        with timed_block("dashboard.load_active_etfs"):
            etf_universe = cached_active_etfs(cache_key, self.etf_universe_store)
        if etf_universe.empty or "ticker" not in etf_universe.columns:
        ...
        self._render_tab_safe(
            "Dashboard", self.dashboard_page.render, etf_universe, self._render_tab_safe
        )

    def _render_shell(self, etf_universe) -> None:
...
def run_app():
    ...
    app = DashboardApp(
        dependencies["etf_universe_store"],
