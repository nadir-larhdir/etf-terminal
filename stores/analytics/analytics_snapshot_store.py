...
class AnalyticsSnapshotStore:
    ...
    def upsert_snapshot(self, snapshot: ETFAnalyticsSnapshot, *, as_of_date: str) -> None:
        ...
        INSERT INTO {qualified_table(self.engine, 'analytics_snapshots')} (
            symbol, as_of_date, asset_bucket, spread_proxy_used,
            estimated_duration, rate_dv01_per_share, cs01_proxy_per_share,
            spread_beta_per_bp, equity_beta, spread_model_r2,
            confidence_level, model_type, model_version,
            computed_from_start_date, computed_from_end_date, notes, reason,
            observations_used, updated_at
        ) VALUES (
            :symbol, :as_of_date, :asset_bucket, :spread_proxy_used,
            :estimated_duration, :rate_dv01_per_share, :cs01_proxy_per_share,
            :spread_beta_per_bp, :equity_beta, :spread_model_r2,
